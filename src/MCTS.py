...
class MCTS:
    def __init__(self, net, c_puct=1.2, n_simulations=120):
        self.net = net
        self.c_puct = c_puct
    ...
    def board_to_tensor(self, board):
        board_state = np.zeros((5, 15, 15), dtype=np.float32)
        board_state[0] = (board.board == board.current_player)
        board_state[1] = (board.board == (3 - board.current_player))
        board_state[2] = (board.board == 0)
        board_state[3] = self.get_consecutive_counts(board, board.current_player)
        board_state[4] = self.get_consecutive_counts(board, 3 - board.current_player)
        return board_state

    def get_consecutive_counts(self, board, player):
        consecutive_counts = np.zeros((15, 15), dtype=np.float32)
        ...
        for row in range(15):
            for col in range(15):
                if board.board[row][col] == player:
                    # 检查水平方向
                    if col <= 10:
                        consecutive_counts[row][col] = sum(board.board[row][col + i] == player for i in range(5))

                    # 检查竖直方向
                    if row <= 10:
                        consecutive_counts[row][col] = max(consecutive_counts[row][col],
                                                           sum(board.board[row + i][col] == player for i in range(5)))

                    # 检查右斜方向
                    if row <= 10 and col <= 10:
                        consecutive_counts[row][col] = max(consecutive_counts[row][col], sum(
                            board.board[row + i][col + i] == player for i in range(5)))
